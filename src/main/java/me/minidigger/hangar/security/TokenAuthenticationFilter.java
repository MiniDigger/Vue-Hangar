...
public final class TokenAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
    ...
    public Authentication attemptAuthentication(
        ...
        final Authentication auth = Optional.ofNullable(param)
                .map(value -> value.replace(BEARER, ""))
                .map(String::trim)
                .map(token -> (Authentication) new UsernamePasswordAuthenticationToken(token, token))
                .orElseGet(() -> new AnonymousAuthenticationToken("key", SpringUser.AnonymousUser.INSTANCE, SpringUser.AnonymousUser.INSTANCE.getAuthorities()));
